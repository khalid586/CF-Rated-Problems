...
void solve(int t){
     ...
     int a[n];
     vector<int> ind;
     rep(i,0,n){ cin >> a[i]; if(!a[i]) ind.push_back(i);}

     rep(i,0,n){
          int nearest = lower_bound(ind.begin(),ind.end(),i) - ind.begin();
          
          int mn = abs(ind[nearest] - i);
          if(nearest) mn = min(mn,abs(ind[nearest-1] - i));
